...
func TestPeek(t *testing.T) {
	...
	t.Run("should return peek error", func(t *testing.T) {
		...
		result, err := Peek(Equal("test"), func(_ string) error { return actionErr }).Matches("test", params)
	...
	t.Run("should execute action before returning provided matcher result", func(t *testing.T) {
		c := ""
		result, err := Peek(Equal("test"), func(v string) error { c = v; return nil }).Matches("test", params)
