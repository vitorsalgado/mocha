...
func TestEither(t *testing.T) {
	...
	t.Run("should return true when only left matcher evaluates to true", func(t *testing.T) {
		result, err := Either(ToEqual("test")).Or(ToContain[string]("qa")).Matches("test", emptyArgs())
		assert.Nil(t, err)
		assert.True(t, result)
	})

	t.Run("should return true when only right matcher evaluates to true", func(t *testing.T) {
		result, err := Either(ToEqual("qa")).Or(ToContain[string]("tes")).Matches("test", emptyArgs())
		assert.Nil(t, err)
		assert.True(t, result)
	})

	t.Run("should return true when both matchers evaluates to true", func(t *testing.T) {
		result, err := Either(ToEqual("test")).Or(ToContain[string]("te")).Matches("test", emptyArgs())
		assert.Nil(t, err)
		assert.True(t, result)
	})

	t.Run("should return false when both evaluates to false", func(t *testing.T) {
		result, err := Either(ToEqual("dev")).Or(ToContain[string]("qa")).Matches("test", emptyArgs())
		assert.Nil(t, err)
