...
func (parser *formURLEncodedParser) CanParse(content string, _ *http.Request) bool {
	return strings.Contains(content, httpval.MIMEFormURLEncoded)
}
...
func (parser *plainTextParser) CanParse(content string, _ *http.Request) bool {
	return strings.Contains(content, httpval.MIMETextPlain) || strings.Contains(content, httpval.MIMEApplicationJSON)
}
