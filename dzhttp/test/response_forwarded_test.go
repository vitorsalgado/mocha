...
func TestProxiedReplies(t *testing.T) {
	...
	scoped := m.MustMock(dzhttp.Post(URLPath("/test")).
		Body(StrictEqual("hello world")).
		Reply(dzhttp.From(dest.URL).
...
func TestProxiedReplyMockFileWithTemplate(t *testing.T) {
	...
	targetScoped := target.MustMock(
		dzhttp.Postf("/test").
			...
			Reply(dzhttp.OK().PlainText("done")))
	...
	scoped := m.MustMock(dzhttp.FromFile("testdata/response_forwarded/proxied_response.yaml"))
