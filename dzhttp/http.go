...
var (
	_ dzstd.MockApp[*HTTPMock] = (*HTTPMockApp)(nil)
)
...
type HTTPMockApp struct {
	*dzstd.BaseApp[*HTTPMock, *HTTPMockApp]
	...
	proxy              *reverseProxy
	templateEngine     dzstd.TemplateEngine
	extensions         map[string]dzstd.Extension
	data               map[string]any
...
func NewAPI(config ...Configurer) *HTTPMockApp {
	...
	var params dzstd.Params
	if conf.Parameters != nil {
		params = conf.Parameters
	} else {
		params = dzstd.NewInMemoryParameters()
	}
	...
	app.BaseApp = dzstd.NewBaseApp[*HTTPMock, *HTTPMockApp](app, store)
	app.server = server
	app.storage = store
...
func (app *HTTPMockApp) Parameters() dzstd.Params {
	return app.params
}
...
func (app *HTTPMockApp) TemplateEngine() dzstd.TemplateEngine {
...
func (app *HTTPMockApp) RegisterExtension(extension dzstd.Extension) error {
	app.rwMutex.Lock()
